...
/** Helper ricorsivo per trovare la prima SurfaceView nel layout corrente */
    ...
    // ==============================================================================
    // V2 ANALOG INTERCEPTOR
    // ==============================================================================
    @Override
    public boolean dispatchGenericMotionEvent(android.view.MotionEvent event) {
        // Check Source: Only intercept if the event is a MOVE action
        if (event.getActionMasked() == android.view.MotionEvent.ACTION_MOVE) {
            ...
            // Event Mutation (CRITICAL): Clone with PointerCoords, force SOURCE_GAMEPAD
            android.view.MotionEvent mutatedEvent = android.view.MotionEvent.obtain(
                    ...
                    event.getDeviceId(),
                    event.getEdgeFlags(),
                    android.view.InputDevice.SOURCE_GAMEPAD, // Strip keyboard labels
                    event.getFlags()
            );
        ...
        if (customKeyMap.containsKey(rawKeyCode)) {
            ...
            if (controllerHandler != null) {
                if (event.getAction() == android.view.KeyEvent.ACTION_DOWN) {
                    ...
                    controllerHandler.handleButtonUp(mutatedEvent);
                }
                return true; // Consume the event so it doesn't leak as a keyboard press
            }
        }

        // Pass unmapped keys through normally
